...
public class VeranstaltungDetailsView {
	...
	public void init()
	{// Initialisierung der Werte f¸r die Detailansicht, wird ¸ber Event ausgelˆst
	...
	{// Reservierung von Tickets zu einer ausgew‰hlten Veranstaltung
		if (this.getAnzTicketsReservierung() > 0)
		{
			...
			{
				FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Leider sind keine " + String.valueOf(getAnzTicketsReservierung()) + " Ticket(s) mehr f√ºr diese Veranstaltung verf√ºgbar." , null);
				FacesContext.getCurrentInstance().addMessage(null, msg);				
			}			
		}else if (this.getAnzTicketsReservierung() == 0)
		{
			FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Sie m√ºssen mindestens ein Ticket zur Reservierung ausw√§hlen." , null);
			FacesContext.getCurrentInstance().addMessage(null, msg);
		}else if (this.getAnzTicketsReservierung() < 0)
		{
			FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Es sind nur positive Zahlen als Anzahl f√ºr zu reservierende Tickets erlaubt." , null);
			FacesContext.getCurrentInstance().addMessage(null, msg);
		}
